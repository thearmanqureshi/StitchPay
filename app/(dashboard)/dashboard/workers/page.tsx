...
export default function WorkersPage() {
  ...
  const handleExport = () => {
    const rows = [
      ["Worker ID", "Name", "Email", "Phone", "Role", "This Month (pcs)", "Status"],
  ...
  return (
    <>
      <Header
        ...
        action={{ label: "Add Worker", onClick: () => { setEditWorker(null); setShowModal(true); } }}
      />
          ...
          <div className="table-card-header">
            <div>
              ...
              <p className="table-card-sub">
                {filteredWorkers.length} {filteredWorkers.length === 1 ? "worker" : "workers"} registered
              </p>
            </div>
          </div>
            ...
            <table className="styles-table">
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={7} className="table-empty">No workers found</td>
                  </tr>
                ) : (
      ...
      {showModal && (
        <AddWorkerModal
          ...
          onSaved={() => { setShowModal(false); fetchWorkers(); }}
        />
      )}
    </>
  );
